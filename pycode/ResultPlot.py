...
for prefix in range(1,11,1):
    orfsvm[prefix-1,:] = np.loadtxt('result/tfRF2L_{0:s}{2:s}{1:s}'.format('Gaussian',str(prefix),mode))
    urfsvm[prefix-1,:] = np.loadtxt('result/tfRF2L_{0:s}{2:s}{1:s}'.format('ReLU',str(prefix),mode))
...
plt.title("Trig vs ReLU NN Nodes on MNIST")
plt.xlabel('sample size (k)')
plt.ylabel('accuracy')
plt.xticks(samplesize/1000)
...
plt.legend(loc=4)
plt.savefig('image/Trig_vs_ReLU_NN.eps')
