...
%% random fourier features method and sparse random features method.
app_results = [];
spa_results = [];
...
for idx = [0:size(features_list,2)-1]
    for jdx = [1:size(C_list,2)]
        for kdx = [0:trials-1]
            x(kdx+1) = app_results(idx*size(C_list,2)*trials+kdx*size(C_list,2)+jdx,1);
            w(kdx+1) = spa_results(idx*size(C_list,2)*trials+kdx*size(C_list,2)+jdx,1);
        end;
        mean_results(idx+1,jdx) = mean(x);
        err_results(idx+1,jdx) = std(x);
        mean_sparse_results(idx+1,jdx) = mean(w);
        err_sparse_results(idx+1,jdx) = std(w);
    end;
end;
...
errorbar(C_list,mean_results(1,:),err_results(1,:),'--bs','MarkerSize',8,'LineWidth',1);
errorbar(C_list,mean_sparse_results(1,:),err_sparse_results(1,:),'--gx','MarkerSize',8,'LineWidth',1);
xlabel('$\log(C)$','interpreter','latex','FontSize',10);ylabel('Classification Accuracy','FontSize',10);
legend({'accurate model','rrf','sparse rff'},'FontSize',10,'Location','southeast');
...
save('mean_sparse_results.mat','mean_sparse_results');
save('err_sparse_results.mat','err_sparse_results');
